"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PublicBookingsController = void 0;
const common_1 = require("@nestjs/common");
const bookings_service_1 = require("../../core/application/bookings.service");
const errors_1 = require("../../core/domain/errors");
let PublicBookingsController = class PublicBookingsController {
    bookings;
    constructor(bookings) {
        this.bookings = bookings;
    }
    cancel(id, token) {
        if (!token) {
            throw errors_1.DomainError.forbidden('INVALID_CANCEL_TOKEN', 'cancelToken is required');
        }
        this.bookings.cancelByGuest(id, token);
    }
};
exports.PublicBookingsController = PublicBookingsController;
__decorate([
    (0, common_1.Delete)(':id'),
    (0, common_1.HttpCode)(204),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Query)('cancelToken')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], PublicBookingsController.prototype, "cancel", null);
exports.PublicBookingsController = PublicBookingsController = __decorate([
    (0, common_1.Controller)('public/bookings'),
    __metadata("design:paramtypes", [bookings_service_1.BookingsService])
], PublicBookingsController);
//# sourceMappingURL=bookings.controller.js.map